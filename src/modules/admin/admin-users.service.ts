...
import { AdminUserResource, toAdminUserResource } from './admin-user.mapper';
...
export interface AdminUserDetail extends AdminUserResource {
...
export class AdminUsersService {
  ...
  async list(perPage: number, cursorToken?: string, status?: string): Promise<Page<AdminUserResource>> {
    ...
    const page = buildPage(rows, perPage, (row) => ({ v: row.createdAt.toISOString(), id: row.id }));
  ...
  async restrict(operatorId: string, id: string, reason: string): Promise<AdminUserResource> {
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.userRestriction.create({
        data: { userId: id, action: 'restrict', reason, userType: existing.userType, adminActor: operatorId },
      });
      return user;
    });
  ...
  async ban(operatorId: string, id: string, reason: string): Promise<AdminUserResource> {
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.userRestriction.create({
        data: { userId: id, action: 'ban', reason, userType: existing.userType, adminActor: operatorId },
      });
      return user;
    });
