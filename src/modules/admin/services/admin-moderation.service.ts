...
export class AdminModerationService {
  ...
  async resolveReport(
    ...
    if (!existing) throw new NotFoundException(AdminResponseMessage.fail.REPORT_NOT_FOUND);

    const report = await this.prisma.contentReport.update({ where: { id }, data: { status } });
    await this.audit.log(operatorId, 'report.resolve', 'ContentReport', id, {
      status,
      note: note ?? null,
    });
    return report;
  ...
  async listAppeals(
    ...
    cursor?: string,
    status?: string,
  ): Promise<Page<RestrictionAppeal>> {
  ...
  async reviewAppeal(
    ...
    const existing = await this.prisma.restrictionAppeal.findUnique({
      where: { id },
      select: { id: true },
    });
    if (!existing) throw new NotFoundException(AdminResponseMessage.fail.APPEAL_NOT_FOUND);

    const appeal = await this.prisma.restrictionAppeal.update({
      where: { id },
      data: { status, adminNote, adminActor: operatorId, reviewedAt: new Date() },
    });
    await this.audit.log(operatorId, 'appeal.review', 'RestrictionAppeal', id, {
      status,
      adminNote,
    });
    return appeal;
  ...
  async approveWaitlist(operatorId: string, id: string): Promise<Waitlist> {
    ...
    if (!existing) throw new NotFoundException(AdminResponseMessage.fail.WAITLIST_ENTRY_NOT_FOUND);
  ...
  async completeDeletionRequest(operatorId: string, id: string): Promise<AccountDeletionRequest> {
    ...
    if (!existing)
      throw new NotFoundException(AdminResponseMessage.fail.DELETION_REQUEST_NOT_FOUND);
