...
import { PriceInput } from '../pricing/pricing.types';
...
export class OrderStateMachine {
  ...
  async apply(orderId: string, transitionName: string, actor: Actor, opts: ApplyOpts = {}): Promise<Order> {
  ...
  private async runAction(
    ...
    switch (t.action) {
      ...
      case 'create-payment-intent': {
        const customer = await tx.stripeCustomer.findUnique({ where: { userId: order.customerId } });
        ...
        await tx.paymentIntent.upsert({
          ...
          update: { stripePiId: res.stripePiId, clientSecret: res.clientSecret, amount: payin, status: res.status },
        });
        return;
      }
      ...
      case 'cancel-payment-intent': {
        ...
        if (pi) {
          ...
          await tx.paymentIntent.update({ where: { orderId: order.id }, data: { status: 'canceled' } });
        }
        return;
      }
      ...
      case 'payout': {
        ...
        await tx.payout.upsert({
          ...
          update: { stripeTransferId: res.stripeTransferId, status: 'paid', releasedAt: new Date() },
        });
        return;
      }
