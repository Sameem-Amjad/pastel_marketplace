...
export class ListingService {
  ...
  async getPublic(id: string): Promise<Listing> {
    ...
    if (!listing) throw new NotFoundException(CatalogResponseMessage.fail.LISTING_NOT_FOUND);
    return listing;
  }
  ...
  async setStock(
    ...
    if (result.count === 0) {
      ...
      if (!exists) throw new ForbiddenException(CatalogResponseMessage.fail.NOT_LISTING_OWNER);
      throw new ConflictException(CatalogResponseMessage.fail.STOCK_VERSION_CONFLICT);
    }
  ...
  private async assertOwner(id: string, authorId: string): Promise<void> {
    ...
    if (!listing || listing.deletedAt)
      throw new NotFoundException(CatalogResponseMessage.fail.LISTING_NOT_FOUND);
    if (listing.authorId !== authorId)
      throw new ForbiddenException(CatalogResponseMessage.fail.NOT_LISTING_OWNER);
  }
}
