...
/**
 * A search result row — deliberately narrower than {@link ListingResource}: it carries only what a
 * grid card renders, so the feed stays cheap to fetch on mobile. Fetch the full listing via
 * `GET /listings/{id}` when the user opens a card.
 */
export class SearchHit {
  ...
  @ApiProperty({ format: 'uuid', description: 'Seller who owns the listing.' })
  authorId!: string;
  ...
  @ApiProperty({
    example: 'https://cdn.mypastel.com/listings/abc.jpg',
    nullable: true,
    description: 'Card image for the grid.',
  })
  primaryImage!: string | null;

  @ApiProperty({ example: 24999, nullable: true, description: 'Price in minor units (cents).' })
  priceAmount!: number | null;

  @ApiProperty({ example: 'GBP', nullable: true })
  ...
  @ApiProperty({
    example: true,
    description: 'Already resolves infinite-stock listings to `true`.',
  })
  inStock!: boolean;
...
export class SearchService {
  ...
  /**
   * Runs a filtered listing search.
   *
   * Steps:
   * 1. Build the WHERE clause from the optional filters, every value bound as a parameter.
   * 2. Resolve the sort, and decode the cursor into a keyset seek on (sortValue, id).
   * 3. Fetch `perPage + 1` rows so the extra row reveals whether another page exists.
   * 4. Split off that row and encode the next cursor.
   */
  async queryListings(q: SearchListingsDto): Promise<Page<SearchHit>> {
    ...
    if (sort === 'relevance' && !hasKeywords) {
      throw new BadRequestException(CatalogResponseMessage.fail.RELEVANCE_REQUIRES_KEYWORDS);
    }
    const { sortSql, sortValExpr, sortType, pgType, comparator } = this.sortPlan(
      sort,
      q.keywords,
      p,
    );
    ...
    if (q.cursor && !cursor)
      throw new BadRequestException(CatalogResponseMessage.fail.INVALID_CURSOR);
    if (cursor) {
  ...
  private sortPlan(
    ...
    switch (sort) {
      case 'createdAt':
        return {
          sortSql: 'created_at ASC, id ASC',
          sortValExpr: 'created_at',
          sortType: 'timestamp',
          pgType: 'timestamptz',
          comparator: '>',
        };
      case 'price':
        ...
        return {
          sortSql: 'price_amount ASC, id ASC',
          sortValExpr: 'price_amount',
          sortType: 'number',
          pgType: 'bigint',
          comparator: '>',
        };
      case '-price':
        return {
          sortSql: 'price_amount DESC, id DESC',
          sortValExpr: 'price_amount',
          sortType: 'number',
          pgType: 'bigint',
          comparator: '<',
      ...
      case 'popularity':
        return {
          sortSql: 'popularity DESC, id DESC',
          sortValExpr: 'popularity',
          sortType: 'number',
      ...
      default:
        return {
          sortSql: 'created_at DESC, id DESC',
          sortValExpr: 'created_at',
          sortType: 'timestamp',
          pgType: 'timestamptz',
          comparator: '<',
